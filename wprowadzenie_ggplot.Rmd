...
temp2 = osoby %>% 
  select(ap4, plec_all, waga_2003_ind, wiek6_2003) %>%
  filter(!is.na(ap4))  %>%
  mutate_each(funs(as_factor), -waga_2003_ind) %>%
  group_by(ap4, plec_all, wiek6_2003) %>%
  summarise(waga = sum(waga_2003_ind, na.rm=TRUE), rok = 2003) %>%
  na.omit() %>% ungroup %>%
  rename("zadowolenie"=ap4, "plec"=plec_all, "wiek_kat"=wiek6_2003)

zycie89 = rbind(temp, temp2) %>% 
  mutate(zadowolenie = factor(tolower(zadowolenie), levels = levels(temp$zadowolenie)[-1], ordered = T))
```
...
Jak pamiętamy, nasze danę dotyczą lat 2003 i 2011. Możemy je rozdzielić za pomocą funkcji *facet_grid*.
...
Na końcu poradźmy sobie z legendą, która nie mieści się na wykresie
...
A może dodatkowo podział na kobiety i mężczyzn?
